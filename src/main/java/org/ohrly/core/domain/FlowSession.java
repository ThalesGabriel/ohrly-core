...
public class FlowSession {
    ...
    public void closeAsSuccess(Instant endedAt) {
        close(FlowSessionEndReason.SUCCESS, endedAt);
    }

    public void closeAsFailure(Instant endedAt) {
        close(FlowSessionEndReason.FAILURE, endedAt);
    }

    public void closeAsHumanHandoff(Instant endedAt) {
        close(FlowSessionEndReason.HANDOFF, endedAt);
    }

    public void closeAsTimeout(Instant endedAt) {
        close(FlowSessionEndReason.TIMEOUT, endedAt);
    }

    private void close(FlowSessionEndReason reason, Instant endedAt) {
        if (isClosed()) {
            throw new IllegalStateException("Session is already closed");
        }

        this.status = FlowSessionStatus.CLOSED;
        this.endReason = Objects.requireNonNull(reason);
        this.endedAt = Objects.requireNonNull(endedAt);
    }

    public boolean isClosed() {
        return status == FlowSessionStatus.CLOSED;
    }

    public boolean isOpen() {
        return status == FlowSessionStatus.OPEN;
    }
